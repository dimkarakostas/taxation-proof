...
amount of assets that it owns is $\Theta = \sum_{i=1}^n bal(\address_i)$, where
$bal(\address)$ is the amount of assets controlled by the address $\address$;
the balance of every address is publicly available on the blockchain.
...
$\taxAuth$ should be $\Theta$, while ensuring a provably correct declaration.
